...
void TileMap::render(sf::RenderTarget &target, const sf::Vector2i &gridPosition)
{
    ...
    for(int x = this->fromX; x <this->toX; x++)
    {
        for(int y = this->fromY; y < this->toY; y++)
        {
            ...
            {
                if(this->map[x][y][this->layer][k]->getType() == TileTypes::ON_TOP)
                {
...
void TileMap::loadFromFile(const std::string file_name)
{
    ...
    if(in_file.is_open())
    {
        ...
        {
            this->map[x][y][z].push_back(new Tile(x, y, gridSizeF, this->tileTextureSheet,
                                                  sf::IntRect(txrx, txry, this->gridSizeI,
                                                              this->gridSizeI), collision, type));
        }
    }
    else
    {
